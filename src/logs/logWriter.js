...
function timestamp() {
    const date = new Date();
    return `${date.getMonth()+1}.${date.getDate()}.${date.getFullYear()} ${date.getHours().toString().padStart(2, '0')}:${date.getMinutes().toString().padStart(2, '0')}:${date.getMilliseconds().toString().substring(0, 2).padStart(2, '0')}`;
}
